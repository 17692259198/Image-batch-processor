...
class Ui_RenameWindow(QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        self.radioButton_4.setGeometry(QtCore.QRect(130, 60, 89, 16))
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.radioButton_4.setText(_translate("MainWindow", "图片名称保存"))
            ...
            for i in range(0, len(self.list)): # 遍历图片列表
                ...
                if os.path.isfile(filepath): # 判断是否为文件
                    ...
                    if imageMark.Ui_MarkWindow().isImg(imgType): # 判断是否为图片
                        ...
                        # 将图片名称添加到列表中
                        image_names.append(self.list[i])

                        # 写入图片名称到txt文件
                    with open("图片名称列表.txt", 'w') as file:
                ...
                if os.path.isfile(filepath):  # 判断是否为文件
                    ...
                    if imageMark.Ui_MarkWindow().isImg(imgType):  # 判断是否为图片
                        if self.radioButton.isChecked():  # 文件名大写
                            ...
                            newfilepath = os.path.join(self.img_path, newfilename)  # 新文件名（包括路径）
                            os.rename(filepath, newfilepath) # 重命名文件
                        elif self.radioButton_4.isChecked():  # txt
                            # 获取或设置txt文件的名称，例如保存为"图片名称列表.txt"
                            txt_filename = "图片名称列表.txt"
                            txt_path = os.path.join(self.img_path, txt_filename)

                            with open(txt_path, 'w', encoding='utf-8') as file:
                                for root, dirs, files in os.walk(self.img_path):
                                    for filename in files:
                                        # 确保只处理图片文件，这里简单示例为检查文件扩展名
                                        if filename.lower().endswith(('.jpg', '.jpeg', '.png', '.gif', '.bmp')):
                                            # 写入文件名到txt（您可以根据需要调整格式）
                                            file.write(filename + '\n')
